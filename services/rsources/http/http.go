...
func NewHandler(service rsources.JobService, logger logger.Logger) http.Handler {
	...
	srvMux.Get("/v1/job-status/{job_run_id}", h.getStatus)
	srvMux.Delete("/v1/job-status/{job_run_id}", h.delete)
	srvMux.Get("/v1/job-status/{job_run_id}/failed-records", h.failedRecords)
	return srvMux
}
